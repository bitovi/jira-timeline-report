...
/**
 * A borderless text button, muted at rest, with a neutral fill under the pointer — a plain
 * `hover:` utility, not the theme's `--section-hover-color`. It's scoped to the button itself, so it
 * doesn't fight the section's own themed hover tint (see `isContainerHovered` in `DocumentEditing`),
 * which still lights up the same as anywhere else in the section.
 */
const AddButton: FC<{ text: string; name?: string; onClick: () => void }> = ({ text, name, onClick }) => (
  <button
    ...
    aria-label={name}
    onClick={onClick}
    className="inline-flex items-center gap-1 rounded px-2 py-1 text-sm text-neutral-801 transition-colors duration-150 hover:bg-neutral-201"
  >
    <AddIcon label="" />
    {text}
  </button>
