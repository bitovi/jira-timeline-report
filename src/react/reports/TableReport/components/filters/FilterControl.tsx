      ...
      return (
        <div className="flex flex-col gap-1">
          <Checkbox
            ...
            onChange={(e) => onChange((e.target as HTMLInputElement).checked ? { kind: 'boolean', value: true } : undefined)}
          />
          <Checkbox
            testId="table-filter-boolean-false"
            label="Is false"
            isChecked={current === false}
            onChange={(e) => onChange((e.target as HTMLInputElement).checked ? { kind: 'boolean', value: false } : undefined)}
          />
        </div>
      );
    }
