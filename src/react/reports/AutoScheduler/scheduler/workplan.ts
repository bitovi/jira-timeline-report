...
export interface ScheduledWorkItem {
  ...
  class ScheduledWorkNode {
    ...
    isPrependPossible(work: WorkItem, firstDayWorkCouldStartOn: number): { isPossible: boolean; possibleStartDay: number } {
  ...
  export class WorkPlans {
    ...
    sortedWorkNodes(): ScheduledWorkNode[] {
      return this.workNodes().sort(
        (a, b) => a.work.startDay - b.work.startDay
      );
    }
  
    sortedWorkNodesByEndDate(): ScheduledWorkNode[] {
      return this.workNodes().sort((a, b) =>
        WorkPlans.sortByEndDate(a.work, b.work)
      );
    }
