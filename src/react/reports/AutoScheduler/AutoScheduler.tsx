...
import type { StatsUIData, SimulationIssueResult, MinimalSimulationIssueResult, UncertaintyWeight } from "./scheduler/stats-analyzer";
    ...
    >
      ...
      {gridData.timeRanges.map(range => {
            return <div 
                ...
                className="border-neutral-30 border-solid border-x px-1 text-xs truncatea">{range.prettyStart}</div>
        })}
      ...
      <div
        ...
        <div
          ...
          style={{ gridRow: 2, gridColumnStart: "what" }} >
...
const SimulationData: React.FC<{
    ...
    function rangeBorderClasses(){
      if(!hasUrl( issue)) {
        ...
        if(!issue.linkedIssue.derivedTiming.isConfidenceValid ||
            (!issue.linkedIssue.derivedTiming.isStoryPointsValid && !issue.linkedIssue.derivedTiming.isStoryPointsMedianValid)
         ) {
...
function gridUIData(statsUIData: StatsUIData, startDate: Date) {
  ...
  const timeRanges = bestFitRanges(startDate, endDate, 12)  as 
...
function gridifyStatsUIData(statsUIData: StatsUIData, startingRows: number){
    ...
    const plans = statsUIData.teams.map((team, i)=>{
        ...
        const start =  previousGridifiedTeam ? previousGridifiedTeam.style.gridRowStart + previousGridifiedTeam.gridRowSpan + 1 : startingRows;
        ...
        const gridifiedTracks = team.tracks.map((issues, i) => {
            return previousTrack = {
                issues: issues,
                style: {gridRowStart: previousTrack ? previousTrack.style.gridRowStart + previousTrack.gridRowSpan : start+ 1},
...
function makeCurveBetweenPoints(start: {x: number, y: number}, end: {x: number, y: number}, controlDistance: number= 30) {
