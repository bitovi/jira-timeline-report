    ...
    const sortedData = useMemo(() => {
        const sorted = [...rows].sort((a, b) => {
            ...
            if (typeof aValue === "number" && typeof bValue === "number") {
                return sortDirection === "asc" ? sortWithNaNAtTheEnd(aValue, bValue) :
                sortWithNaNAtTheEnd(bValue, aValue);
            }
    ...
    return (
        <div className="mb-4">
          ...
          <div className="grid text-sm"
            ...
            {/* Header row */}
            <div className={headerBase} onClick={() => toggleSort("team")}>Team</div>
            <div className={headerBase} onClick={() => toggleSort("issueCount")}>Issues Count</div>
            <div className={headerBase}>Ignored Issues</div>
            <div className={headerBase} onClick={() => toggleSort("mean")}>Mean (A / E)</div>
            <div className={headerBase} onClick={() => toggleSort("variance")}>Variance</div>
            {ranges.map(range => {
                return <div 
                    style={{
                        gridRow: `1 / span 1`,
                    ...
                    className={headerBase}>{range.prettyStart}</div>
            })}
            ...
            {sortedData.map((row, rowIndex) => (
              <React.Fragment key={row.team}>
                <div className={cellBase+" border-r"}
                    onClick={() => {
                ...
                <div className={cellBase+" border-r text-right"}
                    style={{
                        gridRow: `${rowIndex + 2} / span 1`,
                        gridColumn: `2 / span 1`
                    }}>{row.issueCount}</div>
                <div className={cellBase+" border-r text-right"}
                    style={{
                        gridRow: `${rowIndex + 2} / span 1`,
                        gridColumn: `3 / span 1`
                    }}>{row.ignoredIssues.length}</div>
                <div className={cellBase+" border-r text-right"}
                    style={{
                        gridRow: `${rowIndex + 2} / span 1`,
                        gridColumn: `4 / span 1`
                    }}>{row.mean.toFixed(2)}</div>
                <div className={cellBase+" text-right"}
                    style={{
                        gridRow: `${rowIndex + 2} / span 1`,
                        gridColumn: `5 / span 1`
                    }}>{row.variance.toFixed(2)}</div>
                {effortData.workForTeam[row.team].map((value, index) => (
                        <div key={`${team}/${index}`} className={emptyCell+" text-center min-w-1 min-h-1 py-1"}
                            style={{
...
function getTeamEffortMap(teamTimings: TeamTimings): TeamEffortMap {
    ...
    for(const teamTiming of teamTimings) {
        ...
        for(const [date, timings] of teamTiming.businessDaysToIssueTimings) {
            effortMap.set(date, timings.reduce((prev, cur)=> prev + cur.estimatedTeamDaysPerCalendarDay, 0))
        }
