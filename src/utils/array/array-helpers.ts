...
export function indexByKey<T extends Record<string, any>, K extends keyof T>(items: T[], key: K): Record<T[K], T> {
...
export function groupBy<T, K extends PropertyKey>(items: T[], keyGetter: (item: T) => K): Record<K, T[]> {
  return items.reduce(
    (acc, item) => {
