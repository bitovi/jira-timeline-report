import { DerivedWorkTiming, deriveWorkTiming, WorkTimingConfig } from './work-timing/work-timing';
import { DerivedWorkStatus, getWorkStatus, WorkStatusConfig } from './work-status/work-status';
...
export function normalizeAndDeriveIssues(
  ...
  options: Partial<NormalizeIssueConfig & WorkStatusConfig & WorkTimingConfig> & {
    uncertaintyWeight?: number;
  },
): DerivedIssue[] {
  return issues.map((issue: JiraIssue) => deriveIssue(normalizeIssue(issue, options), options));
}
