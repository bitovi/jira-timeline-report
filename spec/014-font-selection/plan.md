...
1. **The Tailwind `.font-sans`/`.font-mono` utilities bypass the variable.** In the compiled
   `dist/production.css`, `.font-sans` emits a _hardcoded_ stack (not `var(--font-sans)`) and
...
export const getFont = (storage: AppStorage): Promise<string> => storage.get<string>(fontKey, defaultFont);

export const updateFont = (storage: AppStorage, font: string): Promise<void> => storage.update(fontKey, font);
