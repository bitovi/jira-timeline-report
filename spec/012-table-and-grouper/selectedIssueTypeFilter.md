...
- **Waits** for `derivedIssues` to load, then validates the value against the _actual_
...
`writeUrlParam` re-adds it. **This is by design** — the current data pipeline _requires_
...
**Key insight:** `rollupTimingLevelsAndCalculations` already encodes _"the selected type
and everything below it."_ In other words, **`selectedIssueType` is already the "From"
...
- **The "From" already app-wide.** `selectedIssueType` / `primaryIssueType` feeds
  _every_ report's pipeline. Reports already consume "selected level **and below**":
...
Constraint from the user: _"we might not be able to use the `selectedIssueType` param,
or we need to change it to optionally support an array."_ Since a From→To range is two
...
- `rollupTimingLevelsAndCalculations` changes from `slice(fromIndex)` to
  `slice(fromIndex, toIndex + 1)` — the _only_ pipeline change.
...
The re-add is correct _for the required "From" level_ — a report must have a primary
level. Two sub-fixes:
...
2. **"To" (`toIssueType`)**: make it _truly optional_ — when absent, resolve to the
...
- Audit the `selectedIssueType` resolver so removing the _To_ cap does not re-add it, and
  document why _From_ still self-heals.
