...
an _ordered_ array of `TableColumnEntry` (`model/persistence.ts:19-26`), and the
...
1. **Precompiled Tailwind.** The app/Storybook load prebuilt `dist/production.css`
   (MEMORY: `tailwind-precompiled-css`). Any _new_ utility class won't apply at
...
3. **Grouped mode reorders columns already.** When 1D-grouped, `displayColumns`
   ...
   operate on the _persisted_ `columnIds`, not the transient `displayColumns`
...
- **Grip is hover-only — the report must look clean for screenshots.** The grip is
  ...
  row must render _identically to today_ — no reserved gap, no faint glyph. Because
  new Tailwind utilities won't apply (precompiled CSS), implement the reveal in
  `TABLE_STYLES`, e.g.:
  ```css
  [data-testid='table-report'] thead th [data-testid='table-header-drag-handle'] {
    opacity: 0;
  }
  [data-testid='table-report'] thead th:hover [data-testid='table-header-drag-handle'],
  [data-testid='table-report'] thead th [data-testid='table-header-drag-handle']:focus-visible {
    opacity: 1;
  }
  ```
...
- Column _resizing_ (`width` already exists on `TableColumnEntry` but is a
  separate feature).
