...
2. **Icon, key, and summary are three separate columns; the user wants one
   ...
   - (linked) summary + the indentation/caret, and — crucially — wants the choice
     between _hierarchy nesting_ and _plain summary sort_ to be a **property of that
...
3. **The summary should be a link that looks like plain text until hovered.**
   ...
   — the _issue key_ column is the only link. The user wants the summary itself to
...
`buildHierarchyRows` from rolled-up roots, with column sort ordering _within_
...
When hierarchy is on, that column sort orders _within_ siblings
...
rest (the key column). Nothing makes the _summary_ a link, and "look like plain
...
- **Pros:** smallest change — only label/option strings + the route value mapping.
  ...
  `tableColumns`.
- **Cons:** doesn't address the user's deeper idea that nesting belongs _to the
  tree column_. "Flat/Tree" still reads oddly next to "Group by ↓".
...
preference), but keep a _small_ visible affordance so it isn't hidden: e.g. the
...
The column just _absorbs_ icon+key+summary into one cell; `isTree` still gates on
...
selecting a Summary sort (or sorting by any _other_ column) = flat. The global
...
`dist/production.css` and _not_ processed at runtime (see
...
so a _new_ Tailwind utility that isn't already in that build will silently not
...
- **Cons:** need `color: inherit` to _override_ the existing
...
- **Global control vs per-column sort:** OK to _remove_ the "Rows" dropdown
