...
- **Row ordering/nesting** — either:
  ...
  - **Hierarchy** ordering: depth-first by parent chain, then by rank within
    siblings. This is _not_ a separate data mode — it's a sort key plus an
...
- **Grouping (optional, 1D or 2D)** — the only feature that introduces _synthetic_
  rows (group headers) and requires **aggregating a column's value across the
  group's member issues**. 1D = group rows only. 2D = group rows _and_ group
...
- Hierarchy ordering (Section 1) is a separate toggle from column sort: when
  hierarchy is on, column sort (if any) orders _within_ siblings; when off,
  column sort orders the whole flat list.
- When grouped, column sort orders rows _within_ each group; a separate control
...
- Filters AND together and apply to the issue set _before_ grouping, so groups
...
- **Field-axis toggle ("Fields: ↓ Down rows / → Across cols").** Controls how the
  measure fields are laid out _within_ the cross-tab, without changing the data:
...
- This replaces an earlier (wrong) "nest the two groupings down the page" idea;
  vertical/horizontal is about the _measure_ axis, not nesting the groupings.
