...
import { fetchJSON } from "../src/jira-oidc-helpers.ts";
...
export function serverInfoPromise({ jiraHelpers, isLoggedIn }) {
  if (resolve(isLoggedIn)) {
    ...
    return fetchJSON("./examples/bitovi-training-server-info.json");
  }
}

export function configurationPromise({ serverInfoPromise, teamConfigurationPromise }) {
