...
const ConfigurationPanel: FC<{ primaryIssuesOrReleasesObs: CanObservable<Array<RolledUpIssue>> }> = ({
    ...
    <div className="">
      ...
      <table className="w-full">
        ...
        <tbody>
          {issues.map((issue) => {
            return (
              <tr className="my-3 text-lg" key={issue.key}>
                <td>{issue.summary}</td>
                {allTeamNames.map((teamName) => {
                  return (
                    <td key={issue.key + "/" + teamName} className="border border-neutral-201">
                      {round(
                        issue.historicalAdjustedEstimatedTime.find((data) => data.teamName === teamName)
