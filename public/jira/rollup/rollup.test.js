...
test('addChildrenFromGroupedHierarchy', () => {
    ...
    const results = addChildrenFromGroupedHierarchy([
        ...
        [
            { key: "m-2", parentKey: "o-1", releases: [{ key: "SPECIAL:release-123" }] },
            { key: "m-3", parentKey: "o-1" }
        ],
        [
            { key: "i-4", parentKey: "m-2", releases: [{ key: "SPECIAL:release-123" }] },
    ...
    expect(results).toStrictEqual([
        [
            { key: "SPECIAL:release-123", reportingHierarchy: { childKeys: ["m-2"], parentKeys: [], depth: 0 } }
        ],
        [
            {
                key: "m-2", parentKey: "o-1",
                releases: [{ key: "SPECIAL:release-123" }],
                reportingHierarchy: { childKeys: ["i-4", "i-5"], parentKeys: ["SPECIAL:release-123"], depth: 1 }
            },
            {
                key: "m-3", parentKey: "o-1",
                reportingHierarchy: { childKeys: ["i-6", "i-7"], parentKeys: [], depth: 1 }
            }
        ],
        [
            {
