...
export const FEATURE_HISTORICALLY_ADJUSTED_ESTIMATES = defineFeatureFlag("historicallyAdjustedEstimates",`
...
export function addHistoricalAdjustedEstimatedTime(issuesOrReleases, rollupTimingLevelsAndCalculations) {
  ...
  const zipped = zipRollupDataOntoGroupedData(groupedIssues, rolledUpHistoricalAdjustedEstimates, "historicalAdjustedEstimatedTime");
  return zipped.flat();
}
...
export function rollupHistoricalAdjustedEstimatedTime(groupedHierarchy, methodNames, {getChildren}  = {}) {
    ...
    return rollupGroupedHierarchy(groupedHierarchy, {
        ...
        createRollupDataFromParentAndChild(issueOrRelease, children, hierarchyLevel, metadata){
            return childrenFirstThenParentTeamAdjustment(issueOrRelease, children, hierarchyLevel, metadata,teamAverageEstimatedPointsPerDay, totalDaysPerTeam);
        }
  });
}
...
export function childrenFirstThenParentTeamAdjustment(parentIssueOrRelease, childrenRollups,hierarchyLevel, metadata,teamAverageEstimatedPointsPerDay, totalDaysPerTeam){
    let data = [];
    if(hierarchyLevel === 0) {
        return {};
    }
...
function issueWasEstimatedDatedAndCompleted(parentIssueOrRelease) {
    const hasSomeEstimates = parentIssueOrRelease.derivedTiming.isStoryPointsMedianValid || parentIssueOrRelease.derivedTiming.isStoryPointsValid;
...
function addEstimatedAndActualForTeam(metadata, parentIssueOrRelease) {
    ...
    compareDataForTeam.push({startDate, dueDate, 
  ...
  ranges.forEach(({ startDate, dueDate, deterministicTotalDaysOfWork, totalDaysOfWork }) => {
      ...
      while (current <= dueDate) {
          ...
          if (dayOfWeek >= 1 && dayOfWeek <= 5) {
            ...
            businessDays.set(dateString,  typeof currentPoints === "number" ? currentPoints+estimatePointsPerDay : estimatePointsPerDay );
          }
...
function getTeamAverageEstimatedPointPerDay(epics) {
    ...
    console.log({baselineEpics: epicsToUseAsBaseline, baselineEpicsCapacities: values, averageEpicDaysOfTeamCapacity, teamAverageEstimatedPointsPerDay});
...
function mergeTeamData(epics) {
    ...
    for(let epic of epics) {
        if(!teamData[epic.teamName]) {
            teamData[epic.teamName] = {userSpecifiedValues: true, historicalAdjustedEstimatedTime: 0, teamName: epic.teamName}
        }
        teamData[epic.teamName].historicalAdjustedEstimatedTime += epic.historicalAdjustedEstimatedTime;
        teamData[epic.teamName].userSpecifiedValues = teamData[epic.teamName].userSpecifiedValues && epic.userSpecifiedValues;
    }
