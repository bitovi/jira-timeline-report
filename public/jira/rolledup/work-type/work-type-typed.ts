...
import {
  ...
  RolledupDatesReleaseOrIssue,
  RollupDateData,
} from "../../rollup/dates/dates-typed";
...
export type WorkTypeTimingReleaseOrIssue = RolledupDatesReleaseOrIssue & {
...
export function rollupDatesByWorkType(
  issuesAndReleases: ReportingHierarchyIssueOrRelease<{
  ...
  const getChildren = makeGetChildrenFromReportingIssues<{
    rollupDates: RollupDateData;
  }>(copies);
  ...
  return copies.map(
    (issue) =>
      ({
        ...issue,
        workTypeRollups: getWorkTypeTimings(issue, getChildren),
      } as WorkTypeTimingReleaseOrIssue)
  );
...
export function getWorkTypeTimings(
  ...
  const workTypeRollupsStaging: {
    [key: string]: { issues: RolledupDatesReleaseOrIssue[] };
  } = {
  ...
  for (let prop in workTypeRollupsStaging) {
    const rollupDates = workTypeRollupsStaging[prop].issues.map(
      (issue) => issue.rollupDates
    );
    workTypeRollups[prop] = mergeStartAndDueData(rollupDates);
    workTypeRollups[prop].issueKeys = workTypeRollupsStaging[prop].issues.map(
      (issue) => issue.key
    );
  }
...
export function mergeParentAndChildIfTheyHaveDates(
  parentIssueOrRelease: IssueOrRelease,
  childRollups: RollupDateData[]
) {
    const self{[ workTypes]:IssueOrRelease}={}
  const rollup = { self: {}, children: {}, combined: {} };
  const derivedParent = parentIssueOrRelease as DerivedIssue;
  const parentData = derivedParent?.derivedTiming;
  ...
  if (hasStartAndDue) {
    const workType = derivedParent.derivedStatus.workType;
    // can use the parent;
    rollup.self[workType] = copyDateProperties(parentData);
    rollup.self[workType].issueKeys = [parentIssueOrRelease.key];
  }
  ...
  for (let workType of workTypes) {
    ...
    if (rollupForWorkType.length) {
      ...
      if (
        hasStartAndDue &&
        parentIssueOrRelease.derivedStatus.workType === workType
      ) {
