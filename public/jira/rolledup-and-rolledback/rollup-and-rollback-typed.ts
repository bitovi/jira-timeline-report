...
import {
  addWorkTypeDates,
  TimingLevelsAndCalculation,
  WorkTypeTimingReleaseOrIssue,
} from "../rolledup/work-type/work-type-typed";
...
import {
  percentComplete as rollupPercentComplete,
  addPercentComplete,
} from "../rollup/percent-complete/percent-complete";
...
export type RolledBackWorkTypeTimingReleaseOrIssue =
  WorkTypeTimingReleaseOrIssue & { issue: RolledBackJiraIssue };

export type IssueOrReleaseWithPreviousTiming = WorkTypeTimingReleaseOrIssue & {
...
export function rollupAndRollback(
  ...
  for (let newIssue of currentStatusRolledUp) {
    ...
    if (oldMap.has(newIssue.key))
      newIssue.issueLastPeriod = oldMap.get(
        newIssue.key
      ) as RolledBackWorkTypeTimingReleaseOrIssue;
  }
...
function addRollups(
  ...
  const normalizedReleases = normalizeReleases(
    derivedIssues,
    rollupTimingLevelsAndCalculations
  );
  const releases = deriveReleases(normalizedReleases);
  const reporting = addReportingHierarchy(
    [...releases, ...derivedIssues] as IssueOrRelease<
      NormalizedIssue | DerivedRelease
    >[],
    rollupTimingLevelsAndCalculations
  );
  const rolledUpDates = addRollupDates(
    reporting,
    rollupTimingLevelsAndCalculations
  );
  ...
  const rolledUpWarnings = rollupWarningIssues(
    rolledUpBlockers,
    rollupTimingLevelsAndCalculations
  );
  const percentComplete = addPercentComplete(
    rolledUpWarnings,
    rollupTimingLevelsAndCalculations
  );
  const childStatuses = rollupChildStatuses(
    percentComplete,
    rollupTimingLevelsAndCalculations
  );
