...
function responseToJSON(response: Response): Promise<JsonResponse> {
	if(!response.ok) {
...
export function nativeFetchJSON(
  url: string,
  options?: RequestInit
...
export default function JiraOIDCHelpers({
	...
	if (CACHE_FETCH) {
      ...
      const localStorageUrl = window.localStorage.getItem(url);
			if (localStorageUrl) {
	...
	const jiraHelpers = {
		saveInformationToLocalStorage: (parameters: { [key: string]: string }) => {
			const objectKeys: string[] = Object.keys(parameters);
		...
		refreshAccessToken: async (accessCode: string): Promise<string> => {
			try {
		...
		fetchAccessibleResources: (passedAccessToken: string): Promise<JsonResponse> => {
		...
		fetchJiraSprint: async (sprintId: string): Promise<JsonResponse> => {
		...
		fetchJiraIssue: async (issueId: string): Promise<JsonResponse> => {
			//this fetches all Recent Projects From Jira
			const scopeIdForJira = jiraHelpers.fetchFromLocalStorage('scopeId');
			const accessToken = jiraHelpers.fetchFromLocalStorage('accessToken');
		...
		editJiraIssueWithNamedFields: async (issueId: string, fields): Promise<string> => {
			const scopeIdForJira = jiraHelpers.fetchFromLocalStorage('scopeId');
			const accessToken = jiraHelpers.fetchFromLocalStorage('accessToken');
