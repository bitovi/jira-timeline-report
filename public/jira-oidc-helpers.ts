import { JsonResponse, JtrEnv, RequestHelperResponse } from "./shared/types.js";
...
export default function (
  ...
  const jiraHelpers: {
    ...
    fetchChildrenResponses: (
      params: { [key: string]: any; fields: string[] },
      parentIssues: Issue[],
      progress?: ((data: ProgressData) => void) | undefined
    ) => Promise<Issue[]>[];
    fetchDeepChildren(
      params: { fields: string[]; [key: string]: any },
      sourceParentIssues: Issue[],
    ...
    fetchAuthorizationCode: () => {
      const url = `https://auth.atlassian.com/authorize?audience=api.atlassian.com&client_id=${JIRA_CLIENT_ID}&scope=${JIRA_SCOPE}&redirect_uri=${JIRA_CALLBACK_URL}&response_type=code&prompt=consent&state=${encodeURIComponent(
    ...
    editJiraIssueWithNamedFields: async (issueId: string, fields: Record<string, any>) => {
      ...
      return fetch(
        `${JIRA_API_URL}/${scopeIdForJira}/rest/api/3/issue/${issueId}?` + "" /*new URLSearchParams(params)*/,
        {
          method: "PUT",
          headers: {
  ...
  jiraHelpers.fetchAllJiraIssuesAndDeepChildrenWithJQLUsingNamedFields = makeDeepChildrenLoaderUsingNamedFields(
  ...
  function mapIdsToNames(obj: { [key: string]: any }, fields: { idMap: { [key: string]: string } }) {
  ...
  function fieldsToEditBody(obj: Record<string, any>, fieldMapping: { nameMap: Record<string, string> }) {
    const editBody: {
      fields: Record<string, any>;
