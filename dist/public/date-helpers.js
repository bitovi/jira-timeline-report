...
export function howMuchHasDueDateMovedForwardChangedSince(epic, checkpointDate) {
    ...
    for (var _i = 0, _a = __spreadArray([], epic.changelog, true).reverse(); _i < _a.length; _i++) {
        var changelog = _a[_i];
...
export function parseDateIntoLocalTimezone(s) {
    ...
    return new (Date.bind.apply(Date, __spreadArray([void 0], ds, false)))();
}
/**
...
export function getLastDateFrom(initiatives, property) {
    ...
    return values.length ? new Date(Math.max.apply(Math, values)) : undefined;
}
export function getDateFromLastPeriod(initiatives, lowercasePhase, checkpoint) {
    ...
    var date = Math.max.apply(Math, dates);
...
export function getFirstDateFrom(initiatives, property) {
    ...
    return values.length ? new Date(Math.min.apply(Math, values)) : undefined;
}
